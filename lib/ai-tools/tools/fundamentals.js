...
const executors = {
  ...
  async calculate_dcf(args) {
      try {
        ...
        // W32（2026-06-10 audit Tier-1 #4）：附三情境估值 — 與 DCF modal 同源
        // （estimateInputs + CAPM suggestWACC），破除 AI 面單點目標價。
        let scenarios = null, scenarioPosition = null, scenarioWacc = null;
        try {
          const sc = dcf.scenarioDCF(stockId, market);
          if (sc.available) {
            scenarios = sc.scenarios;            // {bear,base,bull:{perShare,...}}
            scenarioPosition = sc.position;      // below_range | in_range | above_range
            scenarioWacc = sc.wacc?.wacc ?? null;
          }
        } catch { /* 附加資訊，失敗略過 */ }
        ...
        return {
          ...
          scenarioPosition,
          scenarioWacc,
          scenarioNote: scenarios
            ? '三情境用自動推估 + CAPM 折現率（與 inputs 單點參數可能不同源）；請優先引用「區間」而非單一合理價'
            : null,
