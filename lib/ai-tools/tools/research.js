...
const executors = {
  ...
  async post_trade_review(args = {}) {
    ...
    // 結構化「可偵測的 pattern」— 不用 LLM 也能算的客觀指標
    // 注意：performance 的 shape 由 lib/portfolio-analyst 決定，這裡 best-effort
    const patterns = [];
    if (perf && !perf.error) {
      const winRate = perf.winRate || perf.stats?.winRate || null;
      const avgHold = perf.avgHoldingDays || perf.stats?.avgHoldingDays || null;
      const wins = perf.bestTrades || perf.stats?.bestTrades || perf.topWins || [];
      const losses = perf.worstTrades || perf.stats?.worstTrades || perf.topLosses || [];
      const avgWin = perf.avgWin || perf.stats?.avgWin || null;
      const avgLoss = perf.avgLoss || perf.stats?.avgLoss || null;

      if (typeof winRate === 'number') {
        if (winRate < 0.4) patterns.push({ severity: 'high', label: 'low_win_rate', detail: `勝率 ${(winRate * 100).toFixed(1)}% — 選股或擇時可能有系統性問題` });
        else if (winRate > 0.65) patterns.push({ severity: 'info', label: 'high_win_rate', detail: `勝率 ${(winRate * 100).toFixed(1)}% — 持續維持` });
      }
      if (typeof avgHold === 'number' && avgHold < 5) {
        patterns.push({ severity: 'medium', label: 'over_trading', detail: `平均持有 ${avgHold.toFixed(1)} 天 — 可能過度交易（手續費侵蝕報酬）` });
      }
      if (typeof avgWin === 'number' && typeof avgLoss === 'number' && Math.abs(avgLoss) > avgWin) {
        patterns.push({ severity: 'high', label: 'lopsided_pnl', detail: `平均虧損 ${avgLoss.toFixed(2)} > 平均獲利 ${avgWin.toFixed(2)} — 賺少賠多，停損紀律待加強` });
      }
    }
