...
const definitions = [
  ...
  },
  {
    "type": "function",
    "function": {
      "name": "evaluate_thesis_quality",
      "description": "查某檔的「論點生命週期 / 私有研究履歷」：把使用者過去對這檔的投資論點（建立/更新/失效）、交易紀錄、以及 AI 訊號的事後命中率縫在一起，附描述性自我評估。用來回答『我過去對這檔的判斷準不準 / 論點怎麼演變的』。小樣本不下結論。",
      "parameters": {
        "type": "object",
        "properties": {
          "stockId": { "type": "string", "description": "股票代號" },
          "market": { "type": "string", "description": "TW 或 US" }
        },
        "required": ["stockId", "market"]
      }
    }
  }
];
...
const executors = {
  ...
  async evaluate_thesis_quality(args) {
      const { stockId, market } = args;
      if (!stockId || !market) return { error: 'stockId 與 market 必填' };
      return await callInternal(`/api/thesis/${encodeURIComponent(market)}/${encodeURIComponent(stockId)}/lifecycle`);
    },
