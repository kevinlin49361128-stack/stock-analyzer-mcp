...
const executors = {
  ...
  async detect_kline_patterns(args) {
      try {
        ...
        // 深度學習決策 D2-②：多 K 線幾何型態（雙頂底/頭肩/三角）。描述性、教科書傾向標明非預測。
        let formations = [];
        try {
          const { detectChartPatterns } = require('../../chart-patterns');
          const cp = detectChartPatterns(bars.map(b => ({ ...b, trade_date: b.date })));
          formations = (cp.formations || []).map(f => ({
            name: f.name, label: f.label, descriptive: f.descriptive,
            textbookBias: f.textbookBias, confidence: f.confidence, asOf: f.asOf,
          }));
        } catch (e) { /* 幾何型態失敗不影響 candlestick */ }
        return {
          stockId, market, days,
          totalDetected: enriched.length,
          patterns: enriched,
          formations,                 // 多 K 線幾何型態（描述性）
          formationsNote: '幾何型態為描述性（教科書解讀），非預測、非買賣建議；型態預測力文獻上存疑。',
        };
      } catch (e) {
